...
async function applyManifest(manifest) {
  ...
  if (newPhotos.length === 0 && allPhotos.length > 0) return;
  allPhotos = newPhotos;
...
async function resetPhotos() {
    ...
    try {
        ...
        document.getElementById('login-btn').textContent = '로그인하고 슬라이드 시작';
...
document.getElementById('login-btn').onclick = async () => {
    ...
    try {
        ...
        try {
            ...
            if (allPhotos.length > 0 && !slideshowStarted) startSlideshow();
...
window.onload = async () => {
    ...
    setInterval(async () => {
        try { const m = await fetchManifest(); await applyManifest(m); if (allPhotos.length > 0 && !slideshowStarted) startSlideshow(); } catch {}
    }, MANIFEST_POLL_MS);
};
